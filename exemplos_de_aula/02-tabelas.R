...
base_de_dados_erro <- read_csv2("dados/voos_de_agosto2.csv")
...
# numerico (inteiro ou double), character, logico

# Date

class(base_de_dados$data_hora)

as.POSIXct("2013-01-01T10:00:00Z")
as.Date("2013-01-01")+1
...
mean(base_de_dados$atraso_saida, na.rm = TRUE)
...
class(select(base_de_dados, companhia_aerea))
...
distancia_total_fevereiro <- sum(base_de_dados_fev$distancia)

distancia_total_janeiro

distancia_total_fevereiro
...
as.Date(base_de_dados$data_hora[1])
#as.character(base_de_dados$data_hora[1])
#as.numeric(base_de_dados$data_hora[1])
#as.logical(base_de_dados$data_hora[1])
...
#nascimento_data_br <- as.Date(nascimento_texto_br, format = "%Y-%m-%d") 
...
# as.Date(as.numeric(nascimento_data_br), origin = as.Date("1970-01-01"))
# no Excel o padrão seria as.Date("1899-12-30")
